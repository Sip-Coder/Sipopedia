...
const BASE = "JON_YU_SIP_STUDIES_COMBINED_RESUME_V2_ONE_PAGE_2026-05-25";
const out = {
...
const data = {
  ...
  contact: "Las Vegas, NV | JonYu3@gmail.com | 617.637.0668 | sipstudies.com/resume | sipstudies.com",
  profile:
...
function heading(cmds, x, y, label, width = 400) {
  text(cmds, x, y, label.toUpperCase(), { size: 7.4, bold: true, color: colors.teal2 });
  rect(cmds, x, y + 9.8, width, 0.65, colors.line);
}

function bullets(cmds, x, y, width, rows, size = 6.55) {
  let cursor = y;
  for (const row of rows) {
    text(cmds, x, cursor, "-", { size, bold: true, color: colors.gold });
    cursor = wrapped(cmds, x + 8, cursor, width - 8, [row], { size, color: colors.ink, lineGap: size * 1.15, gap: 0.8 });
  }
  return cursor;
}

function job(cmds, x, y, width, item, size = 5.85) {
  text(cmds, x, y, item.company, { size: 7.25, bold: true, color: colors.ink });
  text(cmds, x, y + 8.2, `${item.role} | ${item.dates}`, { size: 5.95, color: colors.muted });
  return bullets(cmds, x, y + 17, width, item.bullets, size) + 0.5;
}
...
function renderPdfPage() {
  ...
  rect(cmds, 0, 0, 154, page.height, colors.teal);
  image(cmds, state, assets.logo, 24, 22, 106, 56, "contain");
  image(cmds, state, assets.sippy, 21, 94, 38, 54, "cover");
  image(cmds, state, assets.roma, 61, 94, 38, 54, "cover");
  image(cmds, state, assets.hummin, 101, 94, 38, 54, "cover");
  image(cmds, state, assets.scene, 18, 166, 118, 70, "cover");
  text(cmds, 22, 250, "CERTIFICATIONS", { size: 7.3, bold: true, color: colors.pale });
  wrapped(cmds, 22, 266, 112, data.certs, { size: 5.35, color: colors.pale, lineGap: 6.75, gap: 1.8 });
  text(cmds, 22, 432, "SKILLS", { size: 7.3, bold: true, color: colors.pale });
  wrapped(cmds, 22, 448, 112, data.skills, { size: 5.4, color: colors.pale, lineGap: 6.8, gap: 1.55 });
  text(cmds, 22, 610, "FOCUS", { size: 7.3, bold: true, color: colors.pale });
  wrapped(cmds, 22, 626, 112, data.focusAreas, { size: 5.35, color: colors.pale, lineGap: 6.8, gap: 1.5 });
  text(cmds, 22, 724, "EDUCATION", { size: 7.3, bold: true, color: colors.pale });
  wrapped(cmds, 22, 740, 112, [data.education], { size: 5.55, color: colors.pale, lineGap: 6.9 });

  text(cmds, 176, 34, data.name, { size: 24, bold: true, color: colors.teal });
  text(cmds, 178, 62, data.headline, { size: 8.9, bold: true, color: colors.gold });
  text(cmds, 178, 78, data.contact, { size: 6.6, color: colors.muted });
  heading(cmds, 178, 104, "Profile", 392);
  wrapped(cmds, 178, 121, 392, [data.profile], { size: 6.55, color: colors.ink, lineGap: 7.9, gap: 0 });
  heading(cmds, 178, 194, "Experience", 392);
  let y = 209;
  for (const item of data.jobs) y = job(cmds, 178, y + 1, 392, item);
  heading(cmds, 178, 704, "Selected Outcomes", 392);
  const splitAt = Math.ceil(data.outcomes.length / 2);
  const split = [data.outcomes.slice(0, splitAt), data.outcomes.slice(splitAt)];
  bullets(cmds, 178, 721, 188, split[0], 5.95);
  bullets(cmds, 382, 721, 188, split[1], 5.95);
...
function styles() {
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:style w:type="paragraph" w:default="1" w:styleId="Normal"><w:name w:val="Normal"/><w:rPr><w:rFonts w:ascii="Aptos" w:hAnsi="Aptos"/><w:sz w:val="16"/></w:rPr></w:style><w:style w:type="paragraph" w:styleId="Title"><w:name w:val="Title"/><w:rPr><w:b/><w:color w:val="05313B"/><w:sz w:val="32"/></w:rPr></w:style><w:style w:type="paragraph" w:styleId="Heading1"><w:name w:val="Heading 1"/><w:rPr><w:b/><w:color w:val="0B4451"/><w:sz w:val="19"/></w:rPr></w:style><w:style w:type="paragraph" w:styleId="Heading2"><w:name w:val="Heading 2"/><w:rPr><w:b/><w:color w:val="14252B"/><w:sz w:val="17"/></w:rPr></w:style></w:styles>`;
}
...
function core() {
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:title>Jon Yu Combined Resume V2 One Page</dc:title><dc:creator>OpenAI Codex</dc:creator><cp:lastModifiedBy>OpenAI Codex</cp:lastModifiedBy><dcterms:created xsi:type="dcterms:W3CDTF">2026-05-25T00:00:00Z</dcterms:created><dcterms:modified xsi:type="dcterms:W3CDTF">2026-05-25T00:00:00Z</dcterms:modified></cp:coreProperties>`;
}
...
function writeSvg() {
  ...
  svg.push('<rect width="154" height="792" fill="#05313B"/>');
  svg.push(svgImage(assets.logo, 24, 22, 106, 56));
  svg.push(svgImage(assets.sippy, 21, 94, 38, 54));
  svg.push(svgImage(assets.roma, 61, 94, 38, 54));
  svg.push(svgImage(assets.hummin, 101, 94, 38, 54));
  svg.push(svgImage(assets.scene, 18, 166, 118, 70));
  svg.push(svgText(22, 258, "CERTIFICATIONS", { size: 7.3, bold: true, fill: "#E8D9B8" }));
  svg.push(svgWrapped(22, 275, 112, data.certs, { size: 5.35, fill: "#E8D9B8", lineGap: 6.75, gap: 1.8 }).markup);
  svg.push(svgText(22, 440, "SKILLS", { size: 7.3, bold: true, fill: "#E8D9B8" }));
  svg.push(svgWrapped(22, 457, 112, data.skills, { size: 5.4, fill: "#E8D9B8", lineGap: 6.8, gap: 1.55 }).markup);
  svg.push(svgText(22, 618, "FOCUS", { size: 7.3, bold: true, fill: "#E8D9B8" }));
  svg.push(svgWrapped(22, 635, 112, data.focusAreas, { size: 5.35, fill: "#E8D9B8", lineGap: 6.8, gap: 1.5 }).markup);
  svg.push(svgText(22, 732, "EDUCATION", { size: 7.3, bold: true, fill: "#E8D9B8" }));
  svg.push(svgWrapped(22, 749, 112, [data.education], { size: 5.55, fill: "#E8D9B8", lineGap: 6.9 }).markup);
  svg.push(svgText(176, 58, data.name, { size: 24, bold: true, fill: "#05313B" }));
  svg.push(svgText(178, 82, data.headline, { size: 8.9, bold: true, fill: "#C4913C" }));
  svg.push(svgText(178, 98, data.contact, { size: 6.6, fill: "#53696F" }));
  svg.push(svgText(178, 112, "PROFILE", { size: 7.4, bold: true, fill: "#0B4451" }));
  svg.push(svgWrapped(178, 130, 392, [data.profile], { size: 6.55, fill: "#14252B", lineGap: 7.9, gap: 0 }).markup);
  svg.push(svgText(178, 203, "EXPERIENCE", { size: 7.4, bold: true, fill: "#0B4451" }));
  let y = 218;
  for (const item of data.jobs) {
    svg.push(svgText(178, y, item.company, { size: 7.25, bold: true }));
    y += 8.2;
    svg.push(svgText(178, y, `${item.role} | ${item.dates}`, { size: 5.95, fill: "#53696F" }));
    y += 8.8;
    for (const bullet of item.bullets) {
      svg.push(svgText(178, y, "-", { size: 5.85, bold: true, fill: "#C4913C" }));
      const wrappedBullet = svgWrapped(186, y, 384, [bullet], { size: 5.85, fill: "#14252B", lineGap: 6.75, gap: 0.8 });
      svg.push(wrappedBullet.markup);
      y = wrappedBullet.y;
    }
    y += 1.6;
  }
  svg.push(svgText(178, 713, "SELECTED OUTCOMES", { size: 7.4, bold: true, fill: "#0B4451" }));
  const splitAt = Math.ceil(data.outcomes.length / 2);
  svg.push(svgWrapped(178, 730, 188, data.outcomes.slice(0, splitAt).map((x) => `- ${x}`), { size: 5.95, fill: "#14252B", lineGap: 6.9, gap: 1 }).markup);
  svg.push(svgWrapped(382, 730, 188, data.outcomes.slice(splitAt).map((x) => `- ${x}`), { size: 5.95, fill: "#14252B", lineGap: 6.9, gap: 1 }).markup);
  svg.push("</svg>");
