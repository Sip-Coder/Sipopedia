...
test("admin launch gate requires meaningful Stripe and access proof", () => {
  ...
  assert.match(adminSource, /buildFirstDollarAnswerBriefBody/);
  assert.match(adminSource, /Sipopedia First-Dollar Answer Brief/);
  assert.match(adminSource, /Copy answer brief/);
  assert.match(adminSource, /admin_first_dollar_answer_brief_copy/);
  assert.match(adminSource, /Potential Customer Base/);
  assert.match(adminSource, /Homepage Simplification/);
  assert.match(adminSource, /Before First Dollar/);
  assert.match(adminSource, /Current Launch Decision/);
  ...
  assert.match(adminSource, /buildMissingProofChecklistBody/);
  assert.match(adminSource, /Sipopedia Missing First-Dollar Proof Checklist/);
  assert.match(adminSource, /Copy missing proof/);
  assert.match(adminSource, /Missing proof checklist copied/);
  assert.match(adminSource, /admin_launch_missing_proof_copy/);
  assert.match(adminSource, /Do not invite a paid customer until smoke proof, connection proof, and Stripe\/access proof are all clear/);
  assert.match(adminSource, /buildSupabaseProofQueryBody/);
  assert.match(adminSource, /Sipopedia first-dollar Supabase proof query/);
  assert.match(adminSource, /Copy Supabase proof query/);
  assert.match(adminSource, /admin_launch_supabase_proof_query_copy/);
  assert.match(adminSource, /from public\.billing_webhook_events/);
  assert.match(adminSource, /from public\.customer_subscriptions/);
  assert.match(adminSource, /metadata ->> 'stripe_session_id'/);
  assert.match(adminSource, /metadata ->> 'stripe_event_id'/);
  assert.match(adminSource, /metadata ->> 'stripe_subscription_id'/);
  assert.match(adminSource, /One Student subscription row matches the same Stripe session, event, and subscription identifiers/);
  ...
  assert.match(adminSource, /buildFirstPaidTestWorksheetBody/);
  assert.match(adminSource, /Sipopedia First Paid Test Worksheet/);
  assert.match(adminSource, /Copy worksheet/);
  assert.match(adminSource, /admin_first_paid_test_worksheet_copy/);
  assert.match(adminSource, /Rule: use one signed-in production Student account from checkout through unlock and lockout/);
  assert.match(adminSource, /Same Student/);
  assert.match(adminSource, /Same Stripe IDs/);
...
test("pricing page keeps checkout and support choices visible early", () => {
  ...
  assert.match(pricingPageSource, /pricing-buyer-fit-actions/);
  assert.match(pricingPageSource, /pricing-fit-beginner/);
  assert.match(pricingPageSource, /pricing-fit-service/);
  assert.match(pricingPageSource, /pricing-fit-study/);
  assert.match(pricingPageSource, /Save this journey/);
  assert.match(pricingPageSource, /Save service path/);
...
test("readiness checklist documents the same first-dollar proof requirements", () => {
  ...
  assert.match(readinessDoc, /Pricing page lets first customers preview or save the beginner, service, and study paths/);
  ...
  assert.match(readinessDoc, /Copy worksheet/);
  assert.match(readinessDoc, /Student email, saved room, Stripe session, webhook event, Supabase row, screenshots, and lockout proof tied to the same account/);
  assert.match(readinessDoc, /Copy Supabase proof query/);
  assert.match(readinessDoc, /one `billing_webhook_events` event plus one `customer_subscriptions` row share the same Stripe metadata/);
