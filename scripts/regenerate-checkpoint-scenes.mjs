...
function parseMeta(filename) {
  ...
  if (base.endsWith("-aria-v1")) {
    const checkpoint = base.replace("-aria-v1", "").replace(/^winery-/, "");
    return { level: "raw-ingredients", facility: "winery", checkpoint };
  }
...
async function main() {
  ...
  async function worker() {
    ...
    fs.writeFileSync(path.join(outDir, m.filename), Buffer.from(b64, "base64"));
    console.log("ok");
    }
  }
