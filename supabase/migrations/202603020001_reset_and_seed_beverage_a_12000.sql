-- Reset terminology catalog and load beverage-focused A-batch.
-- Target: 12,000 terms total, 1,000 each for:
-- wine, beer, spirits, coffee, tea, kombucha, juice, milk, water,
-- energy drinks, health drinks, powdered supplements.
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'terminology_entries_beverage_type_allowed'
  ) then
    alter table public.terminology_entries
      add constraint terminology_entries_beverage_type_allowed
      check (
        beverage_type in (
          'wine',
          'beer',
...
-- User-requested reset before beverage rebuild.
...
with beverage_tracks as (
  ...
  ) as b(beverage_type, beverage_label, ingredient_focus, reference_links, mla_citations)
),
a_descriptors as (
  select *
  from (
...
select
  ...
  format(
    '"%s %s %s %s" defines a beverage concept focused on %s for %s, emphasizing %s and %s.',
    l.descriptor,
    l.beverage_label,
    l.dimension,
    l.artifact,
  ...
  format(
    'Apply this term by documenting one %s workflow, setting a measurable quality checkpoint, and reviewing outcomes weekly against %s standards.',
    case
      when l.idx % 4 = 0 then 'location'
      when l.idx % 4 = 1 then 'terroir'
  ...
  l.reference_links,
  l.mla_citations,
  'Sip Studies beverage terminology batch A (12,000): generated editorial entries with references and MLA citations.',
  true,
  now()
from limited l
